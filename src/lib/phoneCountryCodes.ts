// International dialing codes (ITU-T E.164 country calling codes) for the
// WhatsApp number field's country-code picker (see
// components/editor/PhoneCountryInput.tsx). WhatsApp/wa.me links only
// work with a full number including the country code — this list exists
// so a creator picks their country once instead of having to know and
// type "237" (or "+237") themselves every time.
//
// Flags are computed from the ISO 3166-1 alpha-2 code (see flagEmoji
// below) rather than hand-typed, so there's one less place a typo could
// silently show the wrong flag next to a correct number.
//
// Cameroon and its immediate neighbors/most-served region are listed
// first (this is where Ringo Connect's own creator base is concentrated
// today — see the hardcoded +237 support number in HelpWidget.tsx) so
// they're reachable without scrolling; everything else follows,
// alphabetically by country name. A creator based anywhere else can still
// pick their own country from the full list — this is a convenience
// default, never a restriction on who can use Ringo Connect.
export type PhoneCountry = { name: string; iso2: string; dial: string };
...
/** 🇨🇲-style flag from an ISO 3166-1 alpha-2 code, via regional indicator symbols. */
...
/**
 * Finds the best (longest-matching) dial code this raw number could start
 * with — e.g. for "23767..." this prefers Cameroon (237) over any
 * single-digit code that also happens to match its first character.
 * Returns null if nothing in the list matches.
 */
export function matchDialCode(rawDigits: string): PhoneCountry | null {
  let best: PhoneCountry | null = null;
  for (const c of PHONE_COUNTRIES) {
    if (rawDigits.startsWith(c.dial) && (!best || c.dial.length > best.dial.length)) {
      best = c;
    }
  }
  return best;
