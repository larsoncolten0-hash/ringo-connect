...
function ensureConfigured() {
  ...
  // The public key must be set as NEXT_PUBLIC_VAPID_PUBLIC_KEY so the
  // client bundle (src/lib/push/subscribeClient.ts) can read it too — the
  // bare VAPID_PUBLIC_KEY fallback exists only for a deployment that set
  // it unprefixed by mistake, since a server-only env var would silently
  // never match what the browser subscribed with otherwise.
