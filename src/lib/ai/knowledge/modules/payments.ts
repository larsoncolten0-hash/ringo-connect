...
Earnings and payouts: music sales earnings (after Ringo's commission and a hold period) are requested as a payout from Dashboard → Music → Earnings; Shop product-sale earnings work the same way from Dashboard → Shop → Earnings (see the "commerce" topic for Shop checkout itself); affiliate commissions are requested from Dashboard → Affiliate. The Ringo team processes payouts, all using the same payout method the owner has on file.
...
`.trim(),
  related: ["plans", "music", "events", "commerce"],
};
