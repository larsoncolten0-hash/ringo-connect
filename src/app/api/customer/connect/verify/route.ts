...
import { EMAIL_RE, connectCustomerToProfile, getConnectableProfile, upsertVerifiedCustomer } from "@/lib/customer/connect";

// Step 2 of Stay Connected: confirm the emailed code. The code check and
// its consumption are ONE atomic database call (row lock, attempt cap,
// email-wide failure ceiling, single use); every failure looks identical
// to the caller. Only after it passes do we create/reuse the customer,
// start the session and record the connection.
