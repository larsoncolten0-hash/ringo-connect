...
// Money disbursement for one Shop payout request, reusing fapshiPayout() as-is — no new Fapshi
// integration. Only applies to XAF payouts (the only currency Shop ever produces) with
// payout_method "mobile_money" — PayPal and bank payouts stay on the manual mark-paid/reject flow
// in ../[id]/route.ts.
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const { data: payout } = await adminClient
    ...
    .select("id, status, amount, currency, payout_method, payout_details, creator_user_id")
    .eq("id", params.id)
    .single();
  ...
  try {
    const result = await fapshiPayout({
      ...
      phone: details.phone,
      medium,
      userId: payout.creator_user_id,
      externalId: `shop-payout-${payout.id}`,
    ...
    await adminClient.from("admin_audit_log").insert({
      admin_id: admin.id,
      action: "send_shop_payout_fapshi",
