...
import type { AdminShopPayoutOverview } from "@/lib/shopPayouts";
import type { ShopPayoutSettings } from "@/lib/shopPayoutSettings";

// Mirrors AdminMusicPayoutsView.tsx closely — same actions (send via Fapshi / check status /
// mark paid / reject), same reasoning, just against commerce_payouts instead of music_payouts.
...
export default function AdminShopPayoutsView({
  overview: initialOverview,
  initialSettings,
}: {
  overview: AdminShopPayoutOverview;
  initialSettings: ShopPayoutSettings;
}) {
  ...
  const resolvePayout = async (id: string, action: "paid" | "rejected") => {
    ...
    setResolvingId(id);
    const res = await fetch(`/api/admin/shop/payouts/${id}`, {
      method: "POST",
  ...
  const sendViaFapshi = async (id: string) => {
    ...
    setSendingId(id);
    const res = await fetch(`/api/admin/shop/payouts/${id}/send`, { method: "POST" });
    const data = await res.json().catch(() => ({}));
    setSendingId(null);
    if (!res.ok) {
  ...
  const checkFapshiStatus = async (id: string) => {
    setCheckingId(id);
    const res = await fetch(`/api/admin/shop/payouts/${id}/check`, { method: "POST" });
    const data = await res.json().catch(() => ({}));
    setCheckingId(null);
    if (!res.ok) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl">
      <div>
        ...
        <h1 className="font-display text-2xl font-medium text-ringo-text tracking-[-0.01em] mb-1">Shop sales payouts</h1>
        <p className="text-sm text-ringo-muted max-w-lg">
          Money owed to sellers from Shop product orders Ringo Connect collected automatically via Fapshi.
        </p>
      </div>
      ...
      {/* Settings live centrally — see Price Controls. */}
      <div className="rounded-card border border-ringo-border/70 bg-ringo-surface p-4 flex items-center justify-between gap-3 flex-wrap">
        <p className="text-sm text-ringo-muted">
          {initialSettings.commercePayoutHoldDays}-day hold · {formatPrice(initialSettings.commerceMinPayoutXaf, "XAF")} minimum
        </p>
        <Link href="/admin/price-controls" className="inline-flex items-center gap-1.5 text-xs font-medium text-ringo-indigo hover:underline">
            ...
            <table className="w-full text-sm min-w-[760px]">
              <thead>
                <tr className="text-left text-ringo-muted border-b border-ringo-border/70">
                  <th className="py-3 px-5 font-normal">Seller</th>
                  ...
                  <tr key={p.id} className="border-b border-ringo-border/40 last:border-0 hover:bg-ringo-muted/5 transition-colors">
                    <td className="py-3 px-5">
                      <p className="text-ringo-text font-medium truncate max-w-[180px]">
                        {p.seller.username ? `@${p.seller.username}` : p.seller.email}
                      </p>
                      <p className="text-xs text-ringo-muted truncate max-w-[180px]">{p.seller.email}</p>
                    </td>
