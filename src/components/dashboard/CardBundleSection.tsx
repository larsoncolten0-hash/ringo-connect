...
export interface CardBundleAddon {
  ...
  grants_plan_name: string;
}

const planLabel = (name: string) => (name ? name.charAt(0).toUpperCase() + name.slice(1) : "");

// Purchase entry point for the Card + Subscription bundles, on the
// existing-user side (/dashboard/ringo-card) — the get-started side offers
// the same rows automatically via the existing addon checkbox list, since
// they're just active addons now (see the migration's own comment).
  ...
  return (
    <div className="rounded-card border border-ringo-border/70 bg-ringo-surface p-5 mb-5">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-sm font-semibold text-ringo-text">{t.cardBundle.title}</h2>
      </div>
      <p className="text-xs text-ringo-muted mb-4">{t.cardBundle.subtitle}</p>

      <div className={`grid gap-3 ${bundles.length >= 3 ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
        {bundles.map((bundle) => (
          <div key={bundle.id} className="rounded-2xl border border-ringo-border p-4 flex flex-col gap-2">
            ...
            <p className="text-xs text-ringo-muted">
              {t.cardBundle.includes(planLabel(bundle.grants_plan_name), bundle.grants_plan_duration_days >= 300 ? t.cardBundle.oneYear : t.cardBundle.oneMonth)}
            </p>
            <button
